...
import type { Grid, MazeProps, Wall } from './../types'
import { randomInt } from 'node:crypto'
import fs from 'node:fs'
import { createCanvas } from 'canvas'
import { directions, E, N, OPPOSITES, S, W, walls } from './../types'
...
export default class Maze {
  ...
  public drawSolution(color = 'blue') {
  ...
  protected initGrid() {
    this.grid = [...new Array(this.width)].map((_, x) => [...new Array(this.height)].map((_, y) => new Bloc(x, y)))
  }
  ...
  protected neighbours(bloc: Bloc): Bloc[] {
    const neighbours = [];
  ...
  protected carvedNeighbours = (bloc: Bloc): Bloc[] =>
    this.neighbours(bloc).filter(n => n.isCarved || n.pos === 0)

  protected uncarvedNeighbours = (bloc: Bloc): Bloc[] =>
